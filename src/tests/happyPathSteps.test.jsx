...
test("Chemin heureux de Configuration Macbook Air", async () => {
  ...
  // Sélectionner "24 Go de mémoire unifiée (250 €)"
  await user.selectOptions(selectElement, "250");
  expect(selectElement.value).toBe("250");
  expect(
    screen.getByRole("option", {
      name: /24 Go de mémoire unifiée/i,
      exact: false,
    }).selected
  ).toBe(true);
  ...
  // "16 Go de mémoire unifiée" => "24 Go de mémoire unifiée"
  ...
  1 199,00 + (250€ de RAM via le précédent select) + (250 SSD avec le clic) = 1659,00€
  */
  ...
  // Vérifie que la liste contient bien les 10 éléments
  ...
  expect(subtotal).toHaveTextContent("1 599,00 €");
